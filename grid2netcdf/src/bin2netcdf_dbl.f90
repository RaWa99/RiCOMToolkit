...
!!=====================================================================
  ...
  PROGRAM bin2netcdf
     ...
     integer :: NE, NP, NTYPE, nsides, NCN, NPR, nnbr, izup, ifront, &
                maxrow, maxsto, j, k, ierr, ncid, istat, nn, ncn2, n1, n2, js
     integer, dimension(8)                :: dimID
     integer, dimension(15)               :: varID
     integer, dimension(:),   allocatable :: IEcode, nbc
     integer, dimension(:,:), allocatable :: nen, ieadj, numsideT, iside, iends

     real(kind=8)                              :: alfa
     real(kind=8), dimension(:),   allocatable :: area, refdep, slen, &
                                                  dlinv, sdx, sdy
     real(kind=8), dimension(:,:), allocatable :: xyz, sxy

  !
  ! Set the name of the binary gridfile to be read
  !--------------------------------------------------------------------
!      GridFileName = 'nzlam061123f_2m_up.lel'
      write(*,*) ' Enter GridFileName (double precision):'
      READ(*,'(a)') GridFileName
  ...
  !--------------------------------------------------------------------
      ...
      allocate( iside(2,nsides), iends(2,nsides) )
...
! *** fill iends if not there
  ...
  !--------------------------------------------------------------------
      ...
      ierr = nf90_def_var( ncid, "XYZ",    nf90_double, &
      ...
      ierr = nf90_def_var( ncid, "SXY",    nf90_double, &
                           (/dimID(5),dimID(3)/), varID(14) )
      ierr = nf90_put_att( ncid, varID(14), "long_name", &
               "X,Y coordinates of midpoint of each side" )

      ierr = nf90_def_var( ncid, "iends",   nf90_int, &
                           (/dimID(5),dimID(3)/), varID(15) )
      ierr = nf90_put_att( ncid, varID(15), "long_name", &
  ...
  !--------------------------------------------------------------------
      ...
      ierr = nf90_put_var( ncid, varID(15), iends )

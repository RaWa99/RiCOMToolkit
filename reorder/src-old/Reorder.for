...
c *** read input data
340   continue
      write(*,*)'Enter filename of triangle list '
	template = 'Triangle file(*.tri),*.tri;All files(*.*),*.*;'
      resOK = OpenFile(12, 'Open triangle file', filnam, template)
...
c
440   continue
      write(*,*)'Enter filename for re-ordered triangle list '
	template = 'Reordered file(*.rtr),*.rtr;All files(*.*),*.*;'
      resOK = OpenFile(14, 'Open triangle file', filnam, template)
c      read(5,99,err=13,end=13) filnam
c      INQUIRE (FILE=FilNam, EXIST=Exis)
c      IF (Exis) THEN
